...
func NewCepController(uc *usecase.CepUsecase) *CepController {
	return &CepController{useCase: uc}
}
...
func (c *CepController) GetClima(ctx *gin.Context) {
	...
	if !cepRegex.MatchString(cepTratado) {
		ctx.JSON(http.StatusUnprocessableEntity, "invalid zipcode")
		return
	}

	out, err := c.useCase.GetCepClima(ctx.Request.Context(), cepTratado)
	if err != nil {
